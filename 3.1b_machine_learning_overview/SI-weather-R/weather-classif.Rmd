...
<<FILL-IN>>         # Get dimensions of train set
...
<<FILL-IN>>(df.tst, "weather-tst.rds")
```
...
df.tst <- <<FILL-IN>>(df.tst,select=-<<FILL-IN>>)                
...
pred.trn <- predict(tree.model,newdata=df.trn,type="class")         # Use model to predict 
...
pred.tst <- <<FILL-IN>>(tree.model,newdata=df.tst,type="class")    
...
paste("Error on TEST Data (Pruned Tree): ", <<FILL-IN>>)                  # Print out error
...
print(<<FILL-IN>>)     # Print model parameters
...
rf.pred.tst <- <<FILL-IN>>(rf.model,newdata=df.tst)                    # Predict test data
...
paste("RF Error on TEST Data: ", <<FILL-IN>>)
```
